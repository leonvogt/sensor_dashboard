...
RSpec.describe RuleViolationHandler do
  ...
  describe '#maybe_create_or_update_violation' do
    it "doesn't create violation if all sensor_measurements are alright" do
      expect {
        RuleViolationHandler.new(device, [sensor_measurement_0, sensor_measurement_1, sensor_measurement_2]).maybe_create_or_update_violation
    ...
    it 'creates a violation if a sensor_measurement is outside the allowed range' do
      ...
      expect {
        RuleViolationHandler.new(device, [sensor_measurement_0, sensor_measurement_1, sensor_measurement_2, sensor_measurement_3]).maybe_create_or_update_violation
    ...
    it 'creates multiple violations if multiple sensor_measurements are outside the allowed range' do
      ...
      expect {
        RuleViolationHandler.new(device, [sensor_measurement_0, sensor_measurement_1, sensor_measurement_2, sensor_measurement_3, sensor_measurement_4]).maybe_create_or_update_violation
      }.to change { RuleViolation.count }.by(2)
    end

    it 'updates an existing violation if there is already one' do
      RuleViolationHandler.new(device, [create(:sensor_measurement, sensor: sensor, value: 21)]).maybe_create_or_update_violation
      expect {
        RuleViolationHandler.new(device, [create(:sensor_measurement, sensor: sensor, value: 25)]).maybe_create_or_update_violation
  ...
  describe '#maybe_close_violation' do
    it "doesn't do anything if there isn't any violation" do
      expect {
        RuleViolationHandler.new(device, [sensor_measurement_0, sensor_measurement_1, sensor_measurement_2]).maybe_close_violation
      }.to_not change { RuleViolation.open.count }
    end

    it 'closes a violation if sensor_measurement is inside the allowed range' do
      RuleViolationHandler.new(device, [create(:sensor_measurement, sensor: sensor, value: 21)]).maybe_create_or_update_violation
      expect {
        RuleViolationHandler.new(device, [sensor_measurement_0, sensor_measurement_1, sensor_measurement_2]).maybe_close_violation
