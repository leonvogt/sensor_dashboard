...
describe NewSensorMeasurementJob, type: :job do
  ...
  it "Saves sensor data" do
    expect {
      NewSensorMeasurementJob.perform_later(device.id, {temperature: 3})
  ...
  describe "sends notifications" do
    ...
    it "sends notifications to users with mobile app connections" do
      expect {
        NewSensorMeasurementJob.perform_later(device.id, {temperature: 30})
